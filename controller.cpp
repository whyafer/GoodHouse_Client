...
void Controller::newConnect() {
    ...
    connect(tcpSocket, SIGNAL(errorOccurred(QAbstractSocket::SocketError)), this,
...
void Controller::zhuceRequest() {
    if (idLine->text->toPlainText() == "")
        ...
        sendMessage(3, idLine->text->toPlainText() + "::==" + mimaLine->text->toPlainText() + "::==" +
                       phoneLine->text->toPlainText() + "::==" + mailLine->text->toPlainText());
        popWindow("请稍后...", false);
    }
}
...
void Controller::xiugaiRequest() {
    if (mimaLine->text->toPlainText() == "")
        ...
        sendMessage(7, mimaLine->text->toPlainText() + "::==" + phoneLine->text->toPlainText() + "::==" +
                       mailLine->text->toPlainText());
    }
}
...
void Controller::fabuRequest() {
    if (titleLine->text->toPlainText() == "") popWindow("标题不能为空");
        ...
        if (fileSize <= 256 * 1024) {
            popWindow("发布中...", false);
            sendMessage(11,
                        ID + "::==" + titleLine->text->toPlainText() + "::==" + addrLine->text->toPlainText() + "::==" +
                        sizeLine->text->toPlainText() + "::==" + loucengLine->text->toPlainText() + "::==" +
                        priceLine->text->toPlainText() + "::==" + psLine->text->toPlainText() + fileString);
